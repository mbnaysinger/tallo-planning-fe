  ...
  <Teleport to="body">
    <div v-if="modelValue" class="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
      ...
      >
        ...
        <div v-if="title" class="mb-4 flex items-start justify-between gap-2">
          <h3 class="text-lg font-bold leading-tight">{{ title }}</h3>
          <button
            class="rounded-full p-1.5 opacity-70 transition-colors hover:bg-muted hover:opacity-100"
            @click="emit('update:modelValue', false)"
          >
            <X class="h-5 w-5" />
          </button>
